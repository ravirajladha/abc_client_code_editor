...
function AccordionItem({ title, children, isOpen, setIsOpen }) {
    return (
        <div className="inline-block items-center mr-10 ml-10 gap-2">
            <button
                className={`rounded-t-[5px] flex-grow text-left py-2 px-4 focus:outline-none border border-gray-300 ${isOpen === title ? 'bg-gray-100 ' : 'bg-white text-gray-700 hover:bg-gray-200'}`}
                onClick={() => setIsOpen(title)}
            >
                {title}
            </button>
            {isOpen === title && <div className="p-1 bg-gray-800 text-white">{children}</div>}
        </div>
    );
}
...
useEffect(() => {
    ...
    const fetchLabDetails = async (labId) => {
        try {
            const response = await fetch(`http://localhost:8000/api/get_lab/${labId}`);
            const data = await response.json();
            
            // Log the full response to verify its structure
            console.log("Full data received from the backend:", data);
            if (data.data && data.data.template) { // Assuming the template is directly inside data.data
        ...
        if (labs && Array.isArray(labs.testcase)) {
            ...
            for (const test of labs.testcase) {
                try {
                    ...
                    const addend1 = test.variables.find(v => v.name === "addend1").value;
                    const addend2 = test.variables.find(v => v.name === "addend2").value;
    ...
    public class Main {
        ...
        public static int addTwoNumbers(int a, int b) {
            return a + b;
                    ...
                    console.log("Final code to execute for test case " + test.id + ":", finalCode);
    ...
    const checkStatus = async (token, test) => {
        ...
        try {
            ...
            if (response.data.stdout) {
                ...
                console.log(`Actual output: '${output.trim()}', Expected output: '${test.expected_output.trim()}'`);
                if (output.trim() === test.expected_output.trim()) {
                    showSuccessToast(`Test case ${test.id} passed!`);
                } else {
                    showErrorToast(`Test case ${test.id} failed! Expected ${test.expected_output}, got ${output}`);
                }
            } else {
            ...
            <nav className='relative flex h-[50px] w-full shrink-0 items-center px-5 bg-white text-dark-gray-7'>
                <div className={`flex w-full items-center justify-between  "max-w-[1200px] mx-auto" `}>
                    ...
                    <div className='flex items-center gap-4 flex-1 justify-center'>
                        ...
                        >
                            <p className="text-black">{labs.name}
                                {/* {console.log(process.env.REACT_APP_RAPID_API_URL)} */}
                            </p>
                        </Link>
                        {/* <div
                    ...
                    <div className='flex items-center space-x-4 flex-1 justify-end '>
                        ...
                        <div className='cursor-pointer group relative'>
                            ...
                            <img src='/avatar.png' alt='Avatar' width={30} height={30} className='rounded-full' onClick={() => notify("The functionality in progress!")} />
                            <ToastContainer />
                            <div
    ...
    <img src="logo.png" alt="Logo" className="w-[300px] h-[300px] bg-cover opacity-500" />
                        ...
                        <div className="relative z-10 flex-1 px-5 ">
                            ...
                            <div className='w-full' >
                                <div className='flex space-x-4'>
                                    <div className='flex-1 mr-2 text-lg text-black font-medium'>{labs.name} </div>
                                </div>
                                ...
                                <div className='mt-4' >
                                    ...
                                    <div className='example-card'>
                                        <pre>
                                            <strong className='text-black'> {accordionContent[openAccordion]} </strong>
                                            <br />
                                        </pre>
                                    </div>
                                </div>
				...
				>
					...
					<div className='preferenceBtn-tooltip'>Settings</div>
                        ...
                        <Split className='h-[calc(100vh-94px)] bg-white' direction='vertical' sizes={[50, 40]} minSize={50}>
